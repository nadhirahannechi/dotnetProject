   ...
   stage('Nexus Upload Stage') {
     ...
     steps { 
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'nexus_manvenuser',usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
               sh 'curl -v -u ${USERNAME}:${PASSWORD} --upload-file publish.tar.gz http://artefact.focus.com.tn:8081/repository/aspNet/com/focuscorp/release-$BUILDVERSION/publish.tar.gz' 
           } 
       } 
   } 
